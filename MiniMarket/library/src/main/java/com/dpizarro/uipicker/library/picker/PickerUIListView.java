 ...
 */
public class PickerUIListView<T extends SimpleTitle> extends ListView {
     ...
     */
    public PickerUIListView(Activity context, List<T> items) {
        super(context);
        if (isInEditMode()) {
            createEditModeView(context);
        } else {
     ...
     */
    public void setItems(Context context, List<T> items, int idRequestPickerUI, int position,
    ...
    public interface PickerUIItemClickListener<T extends SimpleTitle> {
         ...
         */
        void onItemClickItemPickerUI(int which, int position, T valueResult);
    }
}
