...
import com.balinasoft.mallione.adapters.RoutesRecyclerAdapter;
...
public class RoutesFragment extends Basefragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (userListener != null && userListener.getUser() != null) {
            ...
            getService().catigories(requestUser).enqueue(new MyCallbackWithMessageError<ResponseCategories>() {
                @Override
            ...
            getService().catigories().enqueue(new MyCallbackWithMessageError<ResponseCategories>() {
                @Override
                public void onData(ResponseCategories data) {
                    if (thisFragment.isAdded())
                        recyclerAdapter.addList(data.getResult());
