...
import com.balinasoft.mallione.Implementations.Day;
...
//                if () {
                ...
                if (currentDay == null) {
                    ...
                    ApiFactory.getService().addRecord
                            (new RequestAddRecord(userUserListener.getUser().getSession_id()
                                    ,currentDay.getTime()/1000,currentDay.getId(),userUserListener.getUser().getId())).enqueue(new MyCallbackWithMessageError<ResponseAnswer>() {
                        @Override
    ...
    public void addItems() {
        ApiFactory.getService().itemTime(requestItemTime).enqueue(new MyCallbackWithMessageError<ResponseItemTime>() {
            @Override
